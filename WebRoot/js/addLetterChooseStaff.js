	...
	$("#letter-search-staff").datagrid({
		...
		onLoadSuccess : function(){
			...
			$(".selectStaff-button").click(function(){
				...
				$("#letter-add-staff_id").val(rowId);
				$("#letter-add-staff_name").textbox("setValue",rowName);
				$("#addLetterChooseStaff-dialog").dialog("close");
			});
		},
		//让点击无法选中
		onClickCell : function(index){
			$("#letter-search-staff").datagrid("selectRow",index);										
		}
	});
		
	// 给工具条的重置查询按钮绑定点击事件
	$("#letter_tool_undoBtn-staff").click(
			function() {
				// 清空关键字
				$("#letter-search-keywords-staff").textbox("clear");
				// 利用jquery的函数触发查询按钮的点击事件
				$('#letter_tool_searchBtn-staff').trigger('click');
	});
	
	
	//设置关键字
	$("#letter-search-keywords-staff").textbox({
		prompt : "员工姓名|员工编号|电话"
	});
	
	// 设置查询按钮
	$("#letter_tool_searchBtn-staff").click(
		function() {
			if ($("#letter_tool-staff").form("validate")) {
				$("#letter-search-staff").datagrid(
					"load",
					{
						// datagrid的load方法通常用于额外传递参数进行查询
						//这里虽然只有根据关键字进行查询，但是其他不写有可能会空指针
						"queryCriteria.keywords" : $("#letter-search-keywords-staff").textbox("getValue"),
